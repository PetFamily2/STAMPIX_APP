...
const EVENT_LABEL: Record<TeamHistoryRow['eventType'], string> = {
  invite_created: 'נוצרה הזמנה',
  invite_cancelled: 'הזמנה בוטלה',
  invite_accepted: 'הזמנה התקבלה',
  invite_expired: 'הזמנה פגה',
  role_changed: 'תפקיד עודכן',
  suspended: 'עובד הושהה',
  reactivated: 'עובד הופעל מחדש',
  removed: 'עובד הוסר',
  auto_disabled_by_plan: 'גישה לצוות הוגבלה',
  auto_invites_cancelled_by_plan: 'הזמנות בוטלו אוטומטית',
  reinvited_after_removal: 'עובד חזר לאחר הסרה',
};
...
function describeHistoryEvent(row: TeamHistoryRow) {
  switch (row.eventType) {
    case 'invite_created': {
      ...
      return role
        ? `נוצרה הזמנה לתפקיד ${ROLE_LABEL[role]}`
        : 'נוצרה הזמנה חדשה';
    }
    case 'invite_cancelled':
      return 'הזמנה קיימת בוטלה.';
    ...
    case 'invite_expired':
      return 'הזמנה פגה ולא ניתנת עוד לשימוש.';
    case 'role_changed': {
      if (row.fromRole && row.toRole) {
        return `תפקיד שונה מ-${ROLE_LABEL[row.fromRole]} ל-${ROLE_LABEL[row.toRole]}`;
      }
      return 'תפקיד עובד עודכן.';
    }
    case 'suspended':
      return 'העובד הועבר למצב בהשהיה.';
    case 'reactivated':
      return 'העובד הוחזר למצב פעיל.';
    case 'removed':
      return 'העובד הוסר מהצוות.';
    case 'reinvited_after_removal': {
      if (row.fromRole && row.toRole) {
        return `חזר לצוות: ${ROLE_LABEL[row.fromRole]} ל-${ROLE_LABEL[row.toRole]}`;
      }
      return 'עובד חזר לצוות לאחר הסרה.';
    }
    case 'auto_disabled_by_plan':
      return 'גישה לצוות הוגבלה בגלל מצב המנוי.';
    case 'auto_invites_cancelled_by_plan':
      return 'הזמנות ממתינות בוטלו בגלל מצב המנוי.';
    default:
      return 'אירוע צוות עודכן.';
  }
}
...
export default function BusinessTeamManagementScreen() {
  ...
  const managementRows = useMemo(() => {
    const rows: Array<{ label: string; value: string }> = [
      { label: 'עובדים פעילים', value: String(activeRows.length) },
      { label: 'עובדים בהשהיה', value: String(suspendedRows.length) },
      { label: 'עובדים שהוסרו', value: String(removedRows.length) },
      { label: 'הזמנות ממתינות', value: String(pendingInvites.length) },
    ];
      ...
      >
        ...
        >
          <BusinessScreenHeader
            title="ניהול עובדים"
            subtitle="ניהול הרשאות, סטטוסים ופעילות צוות"
        ...
        >
          ...
          >
            <View className={`${tw.flexRow} items-center justify-center gap-2`}>
              ...
              <Text className="text-sm font-black text-white">הוסף עובד</Text>
            </View>
          </TouchableOpacity>
          ...
          <View className="mt-4 rounded-3xl border border-[#E3E9FF] bg-white p-5">
            ...
            >
              <Text
                className={`text-[11px] font-semibold text-[#64748B] ${tw.textStart}`}
              >
                עובדים בהשהיה ({suspendedRows.length})
              </Text>
              <Ionicons
          ...
          <View className="mt-4 rounded-3xl border border-[#E3E9FF] bg-white p-5">
            ...
            >
              <Text
                className={`text-[11px] font-semibold text-[#64748B] ${tw.textStart}`}
              >
                היסטוריית עובדים ({removedRows.length})
              </Text>
              <Ionicons
                        ...
                        <View className={`${tw.flexRow} items-start gap-3`}>
                          ...
                          <View className="flex-1 items-end">
                            ...
                            <Text
                              className={`mt-1 text-xs text-[#64748B] ${tw.textStart}`}
                            >
                              נוצרה ב-{formatDate(invite.createdAt)} · פג תוקף{' '}
          ...
          <View className="mt-4 rounded-3xl border border-[#E3E9FF] bg-white p-5">
            ...
            >
              <Text
                className={`text-[11px] font-semibold text-[#64748B] ${tw.textStart}`}
              >
                יומן פעילות עובדים ({history.length})
              </Text>
              <Ionicons
                ...
                {history.length === 0 ? (
                  ...
                  history.map((row) => (
                    ...
                    >
                      ...
                      >
                        יעד:{' '}
                        {row.targetDisplayName ?? row.targetEmail ?? 'לא זמין'}
                      </Text>
