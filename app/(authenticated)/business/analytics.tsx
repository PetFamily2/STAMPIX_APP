...
const formatWeekLabel = (timestamp: number) => {
  ...
  const startLabel = start.toLocaleDateString('he-IL', { day: '2-digit', month: 'short' });
  const endLabel = end.toLocaleDateString('he-IL', { day: '2-digit', month: 'short' });
...
export default function BusinessAnalyticsScreen() {
  ...
  const dailyBarMax = useMemo(() => {
    ...
    return Math.max(1, ...daily.map((period) => Math.max(period.stamps, period.redemptions)));
  }, [daily]);

  const weeklyBarMax = useMemo(() => {
    if (!weekly.length) return 1;
    return Math.max(1, ...weekly.map((period) => Math.max(period.stamps, period.redemptions)));
  }, [weekly]);
      ...
      <SafeAreaView className="flex-1 bg-[#E9F0FF]" edges={[]}>
        <View className="flex-1 items-center justify-center px-6">
          ...
          <Text className="text-sm text-zinc-400 text-center mb-6">
            התכנית שלך ({SUBSCRIPTION_PLAN_LABELS[subscriptionPlan]}) לא כוללת את הפיצ'ר הזה.
          </Text>
          <TouchableOpacity
      ...
      <ScrollView className="flex-1">
        <View className="max-w-3xl w-full mx-auto px-6 py-8 space-y-6">
          ...
          <View className="grid gap-3">
            {summaryCards.map((card) => (
              ...
              >
                ...
                <Text className="text-xs uppercase tracking-[0.3em] text-zinc-400 mt-1">{card.label}</Text>
                <Text className="text-[11px] text-zinc-500 mt-1">{card.helper}</Text>
              </View>
            ))}
          </View>
          ...
          <View className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4 space-y-3">
            <Text className={`text-lg font-bold text-white ${tw.textStart}`}>גרף יומי</Text>
            {daily.length === 0 ? (
              <Text className="text-xs text-zinc-500">אין עדיין נתונים להצגה.</Text>
            ) : (
              daily.map((period) => (
                <View key={period.start} className="space-y-2">
                  <View className="flex-row items-center justify-between">
                    <Text className="text-sm text-zinc-400">{formatDayLabel(period.start)}</Text>
                    <Text className="text-xs text-zinc-500">
                      {formatNumber(period.stamps)} ניקובים • {formatNumber(period.redemptions)} הטבות
                    </Text>
                  </View>
                  <View className="space-y-1">
                    <View className="h-2 rounded-full bg-zinc-900 overflow-hidden">
                      <View
                        ...
                        style={{ width: `${Math.min((period.stamps / dailyBarMax) * 100, 100)}%` }}
                      />
                    </View>
                    <View className="h-2 rounded-full bg-zinc-900 overflow-hidden">
                      <View
                        className="h-full bg-amber-400"
                        style={{ width: `${Math.min((period.redemptions / dailyBarMax) * 100, 100)}%` }}
                      />
                    </View>
                  </View>
                </View>
          ...
          <View className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4 space-y-3">
            <Text className={`text-lg font-bold text-white ${tw.textStart}`}>גרף שבועי</Text>
            {weekly.length === 0 ? (
              <Text className="text-xs text-zinc-500">הנתונים עדיין נטענים.</Text>
            ) : (
              weekly.map((period) => (
                <View key={period.start} className="space-y-2">
                  <View className="flex-row items-center justify-between">
                    <Text className="text-sm text-zinc-400">{formatWeekLabel(period.start)}</Text>
                    <Text className="text-xs text-zinc-500">
                      {formatNumber(period.uniqueCustomers)} לקוחות • {formatNumber(period.stamps)} ניקובים
                    </Text>
                  </View>
                  <View className="space-y-1">
                    <View className="h-2 rounded-full bg-zinc-900 overflow-hidden">
                      <View
                        ...
                        style={{ width: `${Math.min((period.stamps / weeklyBarMax) * 100, 100)}%` }}
                      />
                    </View>
                    <View className="h-2 rounded-full bg-zinc-900 overflow-hidden">
                      <View
                        className="h-full bg-violet-400"
                        style={{ width: `${Math.min((period.redemptions / weeklyBarMax) * 100, 100)}%` }}
                      />
                    </View>
                  </View>
                </View>
