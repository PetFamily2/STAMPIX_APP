...
const Stamp = ({ filled, label }: { filled: boolean; label: string }) => (
  ...
  >
    <Text
      className={`text-xl font-semibold ${filled ? 'text-amber-300' : 'text-zinc-500'}`}
    >
      {label}
    </Text>
  </View>
...
export default function PreviewCardScreen() {
  ...
  const handleOpenScanner = () => {
    reset();
    router.replace('/(authenticated)/(business)/scanner');
  };
        ...
        <View className="max-w-3xl w-full mx-auto px-6 pb-12 pt-8 space-y-6">
          ...
          <View className="space-y-4">
            <View className="rounded-3xl border border-zinc-800 bg-gradient-to-br from-zinc-900 to-amber-900/30 p-5 shadow-lg shadow-black/50">
              <Text className="text-xs uppercase text-amber-200 tracking-[0.4em]">
                כרטיס נאמנות
              </Text>
              <Text className="mt-2 text-2xl font-bold text-white">
                {businessDraft.name || 'עסק חדש'}
              </Text>
              <Text className="text-sm text-zinc-300">
                {programDraft.rewardName}
              </Text>
          ...
          >
            <Text className="text-center font-bold text-cyan-200">
              פתח סורק
            </Text>
          </TouchableOpacity>

          <Text className="text-[11px] text-zinc-500">
            הפעולה תסיים את תהליך האונבורדינג ותעביר את העסק למסך הסורק שמוכן
            לניהול לקוחות.
          </Text>
        </View>
      </ScrollView>
