...
describe('environment-aware Expo notification configuration', () => {
  ...
  test('native plugins suppress unused photo, microphone, and Face ID declarations', () => {
    const [, secureStoreOptions] = getPlugin(baseConfig, 'expo-secure-store');
    const [, cameraOptions] = getPlugin(baseConfig, 'expo-camera');
    const [, imagePickerOptions] = getPlugin(baseConfig, 'expo-image-picker');
    ...
    const [, locationOptions] = getPlugin(baseConfig, 'expo-location');
    expect(locationOptions).toEqual({
      locationWhenInUsePermission:
        'StampAix uses your approximate location to show nearby participating businesses.',
    ...
    expect(baseConfig.ios.infoPlist.NSLocationDefaultAccuracyReduced).toBe(
      true
    );
    expect(
      baseConfig.ios.infoPlist.NSLocationAlwaysAndWhenInUseUsageDescription
    ).toBeUndefined();
    expect(
      baseConfig.ios.infoPlist.NSLocationAlwaysUsageDescription
    ).toBeUndefined();
  });
