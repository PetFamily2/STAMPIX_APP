...
describe('business onboarding flow helpers', () => {
  ...
  test('returns expected step order and total by flow', () => {
    expect(getBusinessOnboardingStepOrder('plan', 'default')).toBe(10);
    expect(getBusinessOnboardingStepOrder('plan', 'additional')).toBe(3);
    expect(getBusinessOnboardingStepOrder('businessBasics', 'default')).toBe(
      12
    );
    expect(getBusinessOnboardingStepOrder('businessBasics', 'additional')).toBe(
      5
    );
    expect(getBusinessOnboardingTotalSteps('default')).toBe(13);
    expect(getBusinessOnboardingTotalSteps('additional')).toBe(6);
  });
});
