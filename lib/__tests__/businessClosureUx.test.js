...
describe('business closure and recovery UX', () => {
  test('Business Settings exposes owner-only closure without changing leave-business behavior', () => {
    const source = readSource(ACCOUNT_DATA_SCREEN);

    expect(source).toContain("activeBusiness?.staffRole === 'owner'");
    expect(source).toContain('יהיה ניתן לשחזר את העסק בהמשך');
  ...
  test('everyday Settings and Account Details do not show leave/close as ordinary actions', () => {
    ...
    expect(account).not.toContain('handleLeaveBusiness');
    expect(account).not.toContain('closeBusinessAccount');
    expect(account).not.toContain('handlePermanentBusinessDeletion');
    expect(account).toContain('title="ניהול חשבון ונתונים"');
    expect(account).toContain('BUSINESS_ROUTES.accountData');
  });
  ...
  test('canonical authenticated recovery route lists closed businesses and is mode-independent', () => {
    ...
    expect(layout).toContain('inJoin ||');
    expect(layout).toContain('inBusinessRecovery ||');
