...
describe('business settings information architecture', () => {
  ...
  test('keeps team and owner billing permission-gated without a B2B entry', () => {
    const settings = readSource(SETTINGS_SCREEN);
    expect(settings).toContain('BUSINESS_ROUTES.team');
    expect(settings).toContain('title="צוות והרשאות"');
    expect(settings).not.toContain('title="הזמנת עסקים"');
    expect(settings).not.toContain('canInviteBusinesses');
    expect(settings).not.toContain('invite_businesses === true');
...
describe('business settings shared layout stretch', () => {
  test('SettingsSection stretches children and keeps the title RTL-right', () => {
    ...
    expect(section).toContain('gap: 9');
  ...
  test('SettingsGroup fills the available page-shell width', () => {
    ...
    const column = extractNamedStyle(shell, 'column');
    ...
    expect(column).toContain("alignSelf: 'center'");
    expect(body).not.toContain('alignItems');
    expect(copy).toContain("alignItems: 'stretch'");
    expect(copy).not.toContain('alignItems.start');
