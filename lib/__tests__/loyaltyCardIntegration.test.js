...
function firstCompactCardCall(source) {
  const match = source.match(/<LoyaltyCardCompact\b[\s\S]*?\/>/);
  expect(match).not.toBeNull();
  return match?.[0] ?? '';
}
...
describe('canonical loyalty card integrations', () => {
  ...
  test('keeps program management cards compact and progress-free', () => {
    const source = readSource(PROGRAM_MANAGEMENT_SOURCE);
    const cardCall = firstCompactCardCall(source);
  ...
  test('keeps the customer stamp acknowledgement restrained', () => {
    ...
    expect(source).toContain("stampSuccessBanner: '✅ קיבלת חותמת!'");
  ...
  test('keeps intentional previews explicit and free of legacy implicit progress', () => {
    for (const sourcePath of INTENTIONAL_PREVIEW_SOURCES) {
      ...
      expect(cardCall).toContain('preview');
