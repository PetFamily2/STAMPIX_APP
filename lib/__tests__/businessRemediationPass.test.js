...
describe('customer list and activity presentation contracts', () => {
  ...
  test('customer details uses natural status copy, grouped filters, and compact cards', () => {
    ...
    expect(source).toContain('resolveCustomerContactRecommendation');
    expect(source).toContain('{customerContactAction ? (');
    expect(source).toContain('ביקור אחרון:');
    expect(source).toContain('צור קשר');
    expect(source).toContain('useBottomTabBarHeight');
    expect(source).toContain('Math.max(tabBarHeight, insets.bottom || 0) + 28');
    expect(source).toContain('timelineTextWrap: {');
    expect(source).toContain('minWidth: 0');
    expect(source).toContain('timelineDate: {');
...
describe('legal, billing, deletion, and deep-screen contracts', () => {
  test('legal UI uses human link labels without implementation copy or raw URLs', () => {
    ...
    const mobileDocuments = readSource('lib/mobileLegalDocuments.ts');
    ...
    expect(documentScreen).toContain('מחיקת חשבון באתר');
    ...
    expect(documentScreen).toContain('Linking.openURL(ACCOUNT_DELETION_URL)');
  });
