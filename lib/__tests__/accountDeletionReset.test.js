...
describe('account context reset race safety', () => {
  ...
  test('reset invalidates an in-flight notification preference hydration', async () => {
    ...
    const hydration = hydrateNotificationPreference({
      ...
      readPreference: () => preferenceRead.promise,
      setEnabled: (enabled) => {
  ...
  test('current notification hydration applies the canonical preference', async () => {
    const enabledWrites = [];
  ...
  test('final notification cleanup waits for a stale preference read', async () => {
    const queue = createSerializedAsyncOperationQueue();
    const staleRead = deferred();
    const readStarted = deferred();
    const storage = new Map([['canonical', '1']]);
    let generation = 0;
    let enabled = false;
    ...
    const hydration = queue.enqueue(() =>
      hydrateNotificationPreference({
        ...
        readPreference: async () => {
          ...
          return storage.get('canonical') ?? null;
        },
  ...
  test('outer notification operations guard stale results and finally state', () => {
    ...
    expect(refreshBlock).toMatch(/if\s*\(\s*!isCurrentGeneration\(\)/);
    expect(refreshBlock).toMatch(
      /if\s*\(\s*isCurrentGeneration\(\)\s*\)\s*\{\s*setIsSyncing\(false\);?\s*\}/
    );
    ...
    expect(setEnabledBlock).toMatch(
      /if\s*\(\s*isCurrentGeneration\(\)\s*\)\s*\{\s*setIsSyncing\(false\);?\s*\}/
    );
