...
describe('verify RTL build source script', () => {
  ...
  test('keeps every EAS build behind source and native-config verifiers', () => {
    const wrapper = fs.readFileSync('scripts/eas-run.ps1', 'utf8');
    const packageJson = fs.readFileSync('package.json', 'utf8');
    expect(wrapper).toContain('Invoke-PrebuildGate');
    expect(wrapper).toContain('prebuild:preview');
    expect(wrapper).toContain('prebuild:production');
    expect(wrapper).toContain('Assert-CleanGit');
    expect(wrapper).toContain("$EasArgs[0] -eq 'build'");
    expect(packageJson).toContain(
      '"prebuild:preview": "bun run verify:rtl-native-config && bun run verify:rtl-build-source'
    );
    expect(packageJson).toContain(
      '"prebuild:production": "bun run verify:rtl-native-config && bun run verify:rtl-build-source'
    );
  });
});
