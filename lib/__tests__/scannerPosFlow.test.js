...
const PROGRAM_TILE_SOURCE = 'components/loyalty/LoyaltyProgramTile.tsx';
...
describe('scanner POS program preset', () => {
  ...
  test('program switching is ignored while a transaction is active', () => {
    ...
    for (const phase of activePhases) {
      const current = {
        ...
        phase,
        selectedProgramId: 'program_1',
        session: phase === 'success' || phase === 'reversed' ? null : session,
        result: phase === 'success' || phase === 'reversed' ? result : null,
      };
  ...
  test('an empty eligible-program list clears the preset instead of keeping a stale selection', () => {
    ...
    expect(resolveSameBusinessProgramRefresh('program_1', [])).toBe('stale');
...
describe('scanner POS transaction generation races', () => {
  ...
  test('reset while resolve is pending prevents customer state, commit, success, and error', async () => {
    ...
    const tasks = [
      runResolve(pendingResolve),
      runResolve(pendingResolveFailure),
    ];
  ...
  test('screen passes the original resolve generation into commit and resets on business change', () => {
    ...
    expect(source).toContain("dispatch({ type: 'BUSINESS_CHANGED' });");
...
describe('scanner POS error and capability policy', () => {
  test('terminal product errors never become technical retries', () => {
    ...
    expect(
      classifyPosError(new Error('PROGRAM_NOT_FOUND'), 'commit').kind
    ).toBe('stale_program');
    expect(
  ...
  test('structured Convex entitlement payloads are classified by code', () => {
    expect(
      classifyPosError({ data: { code: 'PLAN_LIMIT_REACHED' } }, 'commit').kind
    ).toBe('entitlement');
  });
...
describe('scanner POS compact presentation contract', () => {
  test('uses a fixed five-column wrapped grid with no horizontal carousel', () => {
    ...
    const metrics = getProgramGridMetrics(332, false);
    expect(metrics.tileWidth * 5 + metrics.gap * 4).toBe(metrics.gridWidth);
    expect(metrics.gridWidth).toBeLessThanOrEqual(332);
  ...
  test('uses canonical themes and the business-selected stamp icon', () => {
    const source = readFileSync(SCANNER_SCREEN_SOURCE, 'utf8');
    const tile = readFileSync(PROGRAM_TILE_SOURCE, 'utf8');

    expect(source).toContain('<LoyaltyProgramTile');
    expect(source).toContain('cardThemeId={program.cardThemeId}');
    expect(source).toContain('stampIcon={program.stampIcon}');
    expect(tile).toContain('resolveCardTheme(cardThemeId)');
    expect(tile).toContain('value={stampIcon}');
  });

  test('selection has a whole-tile border, separate check slot, shadow, and accessibility state', () => {
    const tile = readFileSync(PROGRAM_TILE_SOURCE, 'utf8');
    ...
    expect(tile).toContain('styles.check');
    expect(tile).toContain('accessibilityState={{ selected, disabled, busy }}');
  ...
  test('final stamp and already-full scans converge on one redemption offer', () => {
    ...
    expect(finalStampBranch).toContain("type: 'SHOW_REDEEM_CONFIRMATION'");
    expect(resolvedFullBranch).toContain("type: 'SHOW_REDEEM_CONFIRMATION'");
  ...
  test('keeps compact result, countdown undo, and manual RTL helpers', () => {
    ...
    expect(source).toContain("'נוספה חותמת'");
  ...
  test('keeps the compact tile fully pressable with a non-color selected state', () => {
    ...
    expect(tile).toContain('ellipsizeMode="tail"');
    expect(tilePressable).not.toContain('hitSlop');
    expect(tile).toContain('height: PROGRAM_TILE_HEIGHT');
    expect(tile).toContain('height: PROGRAM_LABEL_HEIGHT');
    expect(tile).toContain("overflow: 'hidden'");
    expect(tile).toContain('width: PROGRAM_ICON_CANVAS_SIZE');
    expect(tile).toContain('size={PROGRAM_ICON_NOMINAL_SIZE}');
