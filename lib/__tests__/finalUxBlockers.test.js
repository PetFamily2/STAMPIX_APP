...
describe('program archive confirmation contract', () => {
  ...
  test('only the confirmed runner invokes archive and copy states the consequences', () => {
    ...
    expect(PROGRAM_SOURCE).toContain('לא יוכלו עוד לצבור חותמות');
    expect(PROGRAM_SOURCE).toContain('או לממש הטבות');
    expect(PROGRAM_SOURCE).toContain('אפשר להפעיל אותה מחדש');
    expect(PROGRAM_SOURCE).not.toContain('לא ניתן כרגע לבטל את הפעולה');
    expect(PROGRAM_SOURCE).toContain(
      "archiveDoneMessage: 'הכרטיסיה אינה זמינה עוד לצבירה או למימוש.'"
    );
  });

  test('archived programs expose a confirmed reactivate action', () => {
    const reactivateButton = sourceBetween(
      PROGRAM_SOURCE,
      "{lifecycle === 'archived' ? (",
      '{details.canDelete ? ('
    );
    ...
    expect(reactivateButton).toContain('onPress={handleReactivate}');
    expect(reactivateButton).not.toContain('runReactivate');
    expect(reactivateButton).not.toContain('unarchiveProgram');
    expect(confirmation).toContain(
      'Alert.alert(TEXT.reactivateConfirmTitle, TEXT.reactivateConfirmMessage'
    );
    expect(confirmation).toContain('void runReactivate();');
    expect(runner).toContain('await unarchiveProgram({');
    expect(PROGRAM_SOURCE).toContain('LOYALTY_THEME_CONFLICT_COPY.reactivate');
    expect(PROGRAM_SOURCE).toContain('entitlementErrorToHebrewMessage');
  });
});
