...
describe('rtl source-of-truth styles', () => {
  ...
  test('keeps known Android screenshot regressions anchored to Hebrew right', () => {
    ...
    const benefitRegionStart = welcome.indexOf('TEXT.featureCustomerTitle');
    const businessFeatureIndex = welcome.indexOf(
      'TEXT.featureBusinessTitle',
      benefitRegionStart
    );
    const benefitRegionEnd = welcome.indexOf(
      'handleGetStarted',
      businessFeatureIndex
    );
    expect(benefitRegionStart).toBeGreaterThan(-1);
    expect(businessFeatureIndex).toBeGreaterThan(benefitRegionStart);
    expect(benefitRegionEnd).toBeGreaterThan(benefitRegionStart);

    const welcomeBenefitRegion =
      benefitRegionStart >= 0 && benefitRegionEnd > benefitRegionStart
        ? welcome.slice(benefitRegionStart, benefitRegionEnd)
        : '';
    // biome-ignore lint/suspicious/noTemplateCurlyInString: assertion checks literal source text.
    const manualRowHelperIndex = welcomeBenefitRegion.indexOf('${tw.flexRow}');
    expect(manualRowHelperIndex).toBeGreaterThan(-1);

    const rowClassStart = welcomeBenefitRegion.lastIndexOf(
      '`',
      manualRowHelperIndex
    );
    const rowClassEnd = welcomeBenefitRegion.indexOf('`', manualRowHelperIndex);
    const benefitRowClass =
      rowClassStart >= 0 && rowClassEnd > manualRowHelperIndex
        ? welcomeBenefitRegion.slice(rowClassStart + 1, rowClassEnd)
        : '';
    expect(benefitRowClass).toContain('items-center');

    const benefitTextEnd = welcomeBenefitRegion.indexOf(
      '</Text>',
      manualRowHelperIndex
    );
    const welcomeBenefitRow =
      manualRowHelperIndex >= 0 && benefitTextEnd > manualRowHelperIndex
        ? welcomeBenefitRegion.slice(
            manualRowHelperIndex,
            benefitTextEnd + '</Text>'.length
          )
        : '';

    const iconArea = welcomeBenefitRow.match(
      /<View\b[^>]*\brounded-full\b[^>]*>/
    );
    const hebrewCopy = welcomeBenefitRow.match(
      /<Text\b[^>]*\btext-right\b[^>]*>/
    );
    expect(iconArea).not.toBeNull();
    expect(hebrewCopy).not.toBeNull();
    expect(iconArea?.index ?? -1).toBeLessThan(hebrewCopy?.index ?? -1);

    expect(welcomeBenefitRegion).not.toMatch(
      /\b(?:tw\.flexRowLtr|tw\.flexRowReverse|flexDirection\.rowReverse)\b/
    );
    expect(benefitRowClass).not.toMatch(/(?:^|[\s"'`])flex-row(?=$|[\s"'`])/);
    expect(welcome).not.toContain('I18nManager');
    expect(welcome).not.toContain('forceRTL');
    expect(welcome).not.toMatch(/\bdirection\s*:\s*['"]rtl['"]/);
  });

  test('keeps Business Settings column stacks stretching under manual RTL', () => {
    const section = readFileSync(
      'components/business-settings/SettingsSection.tsx',
      'utf8'
    );
    const nav = readFileSync(
      'components/business-settings/SettingsNavRow.tsx',
      'utf8'
    );
    const form = readFileSync(
      'components/business-settings/ProfileFieldForm.tsx',
      'utf8'
    );

    expect(section).toContain("alignItems: 'stretch'");
    expect(section).not.toContain('alignItems.start');
    expect(section).toContain("textAlign: 'right'");
    expect(section).toContain("writingDirection: 'rtl'");
    expect(nav).toContain(
      "group: {\n    width: '100%',\n    backgroundColor: SETTINGS_TOKENS.surface"
    );
    expect(nav).toContain('flexDirection: flexDirection.row');
    expect(form).toContain("alignItems: 'stretch'");
    expect(form).not.toContain('alignItems.start');

    for (const source of [section, nav, form]) {
