...
describe('Settings layout and account hierarchy', () => {
  ...
  test('account identity is not repeated and missing phone leads to edit', () => {
    const source = readSource(SETTINGS_ACCOUNT);
    expect(source).not.toContain('<InfoLine');
    expect(source).not.toContain('<AccountProperty');
    expect(source).not.toContain('פרט חסר');
    expect(source).toContain('טלפון אישי לחשבון');
    expect(source).toContain('נשמר בנפרד מהטלפון העסקי');
    expect(source).toContain('setMyPhone');
    expect(source).toContain('הוספת טלפון');
    expect(source).toContain('עריכת טלפון');
...
describe('Campaign management and editor QA contracts', () => {
  ...
  test('management screen uses compact product cards and canonical quota usage', () => {
    ...
    expect(list).not.toContain("limitStatus('maxCampaigns',");
    expect(list).not.toContain('campaignQuotaUsed');
