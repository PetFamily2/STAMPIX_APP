...
const QUICK_ACTIONS =
  'components/business-dashboard/QuickShortcutsGrid.tsx';
...
describe('Android physical QA navigation contracts', () => {
  ...
  test('Settings child fallbacks follow the route hierarchy', () => {
    ...
    expect(referrals).toContain('fallbackHref={BUSINESS_ROUTES.settings}');
...
describe('Settings layout and account hierarchy', () => {
  test('shared grouped surfaces and copy columns keep full width', () => {
    ...
    expect(row).toContain('paddingHorizontal: 14');
...
describe('Loyalty management QA contracts', () => {
  ...
  test('over-limit usage is explicit and never renders progress above 100%', () => {
    ...
    expect(list).toContain('הכרטיסיות הקיימות נשמרו');
    expect(list).toContain('יצירה חסומה');
    expect(usage).toContain('const overage = Math.max(0, safeUsed - safeLimit)');
...
describe('Campaign management and editor QA contracts', () => {
  test('C2C referral quota is named and B2B invitations stay separate', () => {
    const list = readSource(CAMPAIGN_LIST);
    const entitlementSource = readSource('convex/entitlements.ts');
    expect(list).toContain('הפניית לקוחות אחת (C2C)');
    expect(list).toContain('הזמנת עסקים ל-StampAix (B2B)');
    ...
    expect(entitlementSource).toContain("referralConfig.kind === 'b2b'");
  });
  ...
  test('management screen uses compact product cards and canonical quota usage', () => {
    ...
    expect(list).toContain('<ManagementUsageSummary');
    expect(list).toContain("limitStatus('maxCampaigns')");
  ...
  test('Campaign editor shares the Loyalty shell and previews customer output', () => {
    ...
    expect(campaign).toContain('lifecycleLabel="העבר לארכיון"');
...
describe('Scanner fixed-slot presentation', () => {
  test('1, 4, 5, and 10 programs preserve five-slot row math', () => {
    ...
    expect(metrics.tileWidth * 5 + metrics.gap * 4).toBe(347);
  ...
  test('tile, icon canvas, and selected dimensions are invariant', () => {
    ...
    expect(tile).toContain('const PROGRAM_ICON_CANVAS_SIZE = 38');
    expect(tile).toContain('const PROGRAM_ICON_NOMINAL_SIZE = 22');
...
describe('Dashboard and Customers restrained polish', () => {
  test('recommendation and B2B primary CTAs remain high-contrast and accessible', () => {
    ...
    expect(referral).toContain('minHeight: 44');
  });
