...
describe('campaign display terminology', () => {
  ...
  test('legitimate offer and reward terminology remains intact', () => {
    // Loyalty hub create no longer one-tap inserts a sample card. The approved
    // offer promise "אין כפל מבצעים" remains user-facing on the canonical
    // create/editor path, not as campaign navigation copy.
    expect(readSource('lib/loyalty/cardTerms.ts')).toContain('אין כפל מבצעים');
    expect(
      readSource('app/(authenticated)/(business)/cards/new.tsx')
    ).toContain('DEFAULT_LOYALTY_CARD_TERMS');
    expect(
      readSource('app/(authenticated)/(business)/cards/[programId].tsx')
    ).toContain('DEFAULT_LOYALTY_CARD_TERMS');
    expect(readSource('app/(authenticated)/(customer)/rewards.tsx')).toContain(
      'הטבות ומבצעים'
    );
    expect(
      readSource('lib/businessSettings/profileFields.ts')
