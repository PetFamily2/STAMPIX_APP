...
const CAMPAIGN_NAVIGATION_SOURCES = [
...
describe('campaign display terminology', () => {
  ...
  test('campaign navigation labels use קמפיינים instead of מבצעים', () => {
    for (const relativePath of CAMPAIGN_NAVIGATION_SOURCES) {
  ...
  test('legitimate offer and reward terminology remains intact', () => {
    ...
    expect(
      readSource('app/(authenticated)/(customer)/rewards.tsx')
    ).toContain('הטבות ומבצעים');
    expect(
      readSource(
