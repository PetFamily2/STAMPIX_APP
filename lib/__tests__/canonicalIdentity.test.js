...
test('canonical launch identity stays aligned across Expo platforms', () => {
  ...
  assert.equal(appConfig.ios.usesAppleSignIn, undefined);
  assert.equal(appConfig.ios.entitlements, undefined);
  assert.ok(appConfig.plugins.includes('expo-system-ui'));
