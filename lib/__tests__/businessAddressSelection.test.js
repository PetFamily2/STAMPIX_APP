...
describe('business address selection state', () => {
  ...
  test('Settings save confirms success before returning with history-aware back', () => {
    const settingsSource = readFileSync(
      'app/(authenticated)/(business)/settings-business-address.tsx',
      'utf8'
    );
    ...
    const destinationIndex = saveHandler.indexOf(
      'safeBack(BUSINESS_ROUTES.profile)'
    );
    ...
    expect(saveHandler).not.toContain('router.replace(');
