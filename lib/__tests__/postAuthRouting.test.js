...
describe('auth-group authoritative route disposition', () => {
  ...
  test('unresolved authenticated routing inputs render loading without redirect', () => {
    expect(disposition({ postAuthResolution: { status: 'loading' } })).toEqual({
      status: 'loading',
    });
  });
  ...
  test('additional-business onboarding is disabled for MVP', () => {
    expect(
      disposition({
        routeKind: 'businessOnboarding',
        businessOnboarded: true,
    ...
    ).toEqual({ status: 'redirect', href: POST_AUTH_ROUTES.customerWallet });
  });
...
describe('Batch 1 auth and onboarding source contracts', () => {
  ...
  const nameCaptureSource = readFileSync('app/(auth)/name-capture.tsx', 'utf8');
  ...
  test('default business draft hydration gates merchant onboarding but additional flow bypasses it', () => {
    ...
    const firstRedirectIndex =
      merchantOnboardingLayoutSource.indexOf('<Redirect');
  ...
  test('completed default business onboarding is guarded without blocking additional flow', () => {
    ...
    expect(merchantOnboardingLayoutSource).toContain('!isAdditionalFlow');
    expect(authenticatedLayoutSource).toContain(
      '!isAdditionalMerchantOnboarding'
    );
  });

  test('authenticated startup uses only a real loading gate without fake progress', () => {
    expect(authenticatedLayoutSource).toContain('return <FullScreenLoading />');
    expect(authenticatedLayoutSource).not.toContain('loadingProgress');
    expect(authenticatedLayoutSource).not.toContain('loadingPercent');
    expect(authenticatedLayoutSource).not.toContain('setInterval');
    expect(authenticatedLayoutSource).not.toContain('isLoadingPhaseDone');
    expect(authenticatedLayoutSource).not.toContain('טוען את הנתונים שלך');
  });
  ...
  test('name bootstrap and name save failures expose retryable states', () => {
    ...
    expect(nameCaptureSource).toContain(
