...
const FEATURED_BUSINESSES = [
  { id: 'f1', name: 'בית השוקולד', type: 'מתיקה', img: 'https://picsum.photos/400/200?chocolate', offer: 'פרלין מתנה בהצטרפות' },
  { id: 'f2', name: 'המספרה של רוני', type: 'טיפוח', img: 'https://picsum.photos/400/200?hair', offer: '10% הנחה על תספורת ראשונה' },
  { id: 'f3', name: 'פיצה פאמיליה', type: 'אוכל', img: 'https://picsum.photos/400/200?pizza', offer: 'תוספת חינם בהזמנה' },
];
  ...
  return (
    <div className="flex-1 flex flex-col pb-24 overflow-y-auto">
      ...
      <main className="px-5 flex flex-col gap-8 mt-6 animate-in slide-in-from-right duration-300">
        <div>
           <h2 className="text-2xl font-black text-text-main mb-1">גלה עסקים חדשים</h2>
           <p className="text-gray-500 text-sm font-medium">עסקים שמחכים לך ממש מעבר לפינה</p>
        </div>

        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-5 px-5">
           {['קפה', 'מסעדות', 'טיפוח', 'אופנה', 'כושר'].map((cat, i) => (
             <button key={i} className={`whitespace-nowrap px-6 py-2 rounded-full font-bold text-sm ${i === 0 ? 'bg-primary text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100'}`}>
               {cat}
             </button>
           ))}
        </div>
        ...
        <div className="flex flex-col gap-6">
           {FEATURED_BUSINESSES.map(biz => (
             <div key={biz.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-50 flex flex-col active:scale-[0.99] transition-all cursor-pointer">
                ...
                <div className="p-6 flex flex-col gap-4">
                   <div className="flex justify-between items-start">
                      <div>
                         <h3 className="text-xl font-black text-text-main">{biz.name}</h3>
                         <p className="text-primary text-sm font-bold mt-1">{biz.offer}</p>
                      </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-50 flex justify-around items-end pb-8 pt-3 z-10 px-4">
        ...
        <button onClick={() => onNavigate('rewards')} className="flex flex-col items-center gap-1 text-gray-300">
        ...
        <button onClick={() => onNavigate('discovery')} className="flex flex-col items-center gap-1 text-primary">
          <span className="material-symbols-outlined text-[28px] material-symbols-filled">explore</span>
