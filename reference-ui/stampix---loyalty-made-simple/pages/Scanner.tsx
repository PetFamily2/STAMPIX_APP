...
export default function Scanner({ onBack }: Props) {
  const [status, setStatus] = useState<'scanning' | 'success' | 'reward'>('scanning');
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative z-10 px-6">
         {status === 'scanning' && (
           <>
              ...
              <p className="mt-10 text-white font-bold text-lg">מקם את הקוד בתוך המסגרת</p>
              <p className="text-gray-400 text-sm mt-2">סריקה אוטומטית ברגע שהקוד מזוהה</p>
           </>
         )}
         ...
         {status === 'success' && (
           <div className="w-full max-w-sm bg-primary rounded-[2.5rem] p-8 flex flex-col items-center text-center shadow-2xl animate-in zoom-in duration-300">
              ...
              <h3 className="text-3xl font-black text-white mb-2">הסריקה הצליחה!</h3>
              <p className="text-white/80 leading-relaxed mb-10">
                החותמת נוספה בהצלחה.<br/>
         ...
         {status === 'reward' && (
           <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 flex flex-col items-center text-center shadow-2xl animate-in zoom-in duration-300">
              ...
              <h3 className="text-3xl font-black text-text-main mb-2">זכאות לפרס!</h3>
              <p className="text-gray-500 leading-relaxed mb-10">
                מזל טוב! הלקוח צבר 10 חותמות.<br/>
              ...
              <div className="w-full flex flex-col gap-3">
                 ...
                 <button onClick={onBack} className="text-gray-400 font-bold text-sm py-2 hover:text-gray-600">דלג לבינתיים</button>
              </div>
           </div>
         )}
      </main>
