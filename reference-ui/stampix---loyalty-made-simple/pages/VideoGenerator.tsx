...
export default function VideoGenerator({ onBack }: Props) {
  ...
  const handleGenerate = async () => {
    try {
      ...
      const prompt = "A short, stylish promotional video for a loyalty app called STAMPIX. A happy customer shows their digital stamp card on a smartphone to a barista in a modern cafe. Vibrant colors, sunlight, 4k quality, trendy vibe.";
      ...
      if (err?.message?.includes("Requested entity was not found")) {
        setError("נראה שיש בעיה עם מפתח ה-API. אנא בחר מפתח פעיל עם חיוב מופעל.");
        const aistudio = (window as any).aistudio;
        await aistudio.openSelectKey();
      } else {
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center">
        {!loadingStatus && !videoUrl && (
          <div className="space-y-6">
            ...
            <div>
              <h2 className="text-2xl font-black mb-2">צור פרסומת ב-10 שניות</h2>
              <p className="text-gray-500 leading-relaxed">
                הבינה המלאכותית שלנו תיצור עבורך סרטון שיווקי מרהיב לעסק.
                הסרטון ידגים ללקוחות כמה קל להשתמש ב-STAMPIX אצלך.
              </p>
            </div>
          ...
          <div className="space-y-8 w-full">
            <div className="relative size-40 mx-auto">
              ...
              <div className="absolute inset-4 bg-primary/10 rounded-full flex items-center justify-center">
                <span className="material-symbols-outlined text-primary text-4xl animate-pulse">auto_videocam</span>
              </div>
            </div>
            <div>
              <h3 className="text-xl font-bold text-text-main mb-2">הקסם מתרחש...</h3>
              <p className="text-primary font-medium text-lg animate-pulse">{loadingStatus}</p>
            </div>
            <p className="text-gray-400 text-sm">זה עשוי לקחת דקה או שתיים, אל תסגור את האפליקציה</p>
          </div>
        )}
