...
export default function MerchantStoreSettings({ onBack, onNavigate, tier }: Props) {
      ...
      <main className="px-6 flex flex-col gap-8 mt-6">
        ...
        <section className="flex flex-col gap-3">
          <div className="flex justify-between items-center px-1 flex-row-reverse">
             <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest text-right">תצוגה מקדימה ללקוח</h3>
             {isLocked && (
          ...
          <div className={`bg-white rounded-[2.5rem] p-6 shadow-xl border border-gray-100 relative overflow-hidden transition-all text-right ${isLocked ? 'opacity-100 scale-100' : 'opacity-90'}`}>
             <div className={`absolute top-0 right-0 w-full h-2 ${brandColor} transition-colors`}></div>
             <div className="flex items-center gap-4 mb-4 flex-row-reverse">
                ...
                <div>
                   <h4 className="font-bold text-lg text-text-main leading-none">{storeName}</h4>
                   <p className="text-xs text-gray-400 mt-1">צבור חתימות לקבלת {rewardText}</p>
                </div>
             </div>
             
             <div className="flex gap-2 flex-wrap flex-row-reverse">
                {Array.from({ length: maxStamps }).map((_, i) => (
                  <div key={i} className={`size-7 rounded-full flex items-center justify-center border-2 ${i < 3 ? `${brandColor} border-transparent text-white` : 'bg-gray-50 border-gray-100'}`}>
                     {i < 3 ? <span className="material-symbols-outlined text-[14px] font-bold">check</span> : (i === maxStamps - 1 ? <span className="material-symbols-outlined text-xs text-gray-300">redeem</span> : null)}
                  </div>
                ))}
             </div>
          </div>
        ...
        <section className={`bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-50 flex flex-col gap-6 text-right transition-all ${isLocked ? 'opacity-70 pointer-events-none grayscale-[0.5]' : ''}`}>
           <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-gray-700">שם העסק המופיע</label>
              <input 
                type="text" 
                value={storeName}
                disabled={isLocked}
           ...
           <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-gray-700">תיאור ההטבה (למשל: קפה חינם)</label>
              <input 
                type="text" 
                value={rewardText}
                disabled={isLocked}
           ...
           <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-gray-700">כמות ניקובים לכרטיס מלא: <span className="text-blue-600">{maxStamps}</span></label>
              <input 
                type="range" 
                min="3" 
                max="12" 
        ...
        <section className={`bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-50 flex flex-col gap-4 text-right transition-all ${isLocked ? 'opacity-70 pointer-events-none grayscale-[0.5]' : ''}`}>
           ...
           <div className="grid grid-cols-3 gap-3">
              {colors.map((c) => (
                ...
                >
                  <div className={`size-8 rounded-full ${c.class} shadow-sm`}></div>
        ...
        <section className="flex flex-col gap-4">
           <div className="flex justify-between items-center px-1 flex-row-reverse">
              ...
              <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-full uppercase">מנוי {tier}</span>
           </div>
           
           <div 
              onClick={() => (tier === 'starter' ? onNavigate('checkout') : alert('פותח מסך יצירה חדש...'))}
           ...
           >
              ...
              <div>
                 <h5 className="font-black text-lg text-text-main">צור כרטיסיה חדשה</h5>
                 <p className="text-sm text-gray-400 mt-1">מבצע עונתי, Happy Hour או מועדון משני</p>
              </div>
              ...
              {tier === 'starter' && (
                <div className="flex flex-col items-center gap-2 mt-2">
                   ...
                   <p className="text-[10px] text-gray-400">במסלול ה-Starter ניתן לנהל כרטיסיה אחת בלבד</p>
                </div>
              )}
           </div>
        </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 shadow-2xl flex flex-col gap-6 animate-in zoom-in duration-300">
             ...
             <div className="text-center">
                <h3 className="text-2xl font-black text-text-main mb-2">נעילת כרטיסיה?</h3>
                <p className="text-gray-500 text-sm leading-relaxed">
                  מרגע הנעילה לא ניתן יהיה לערוך את שם העסק, כמות הניקובים או הפרס. זאת כדי לשמור על אמינות מול הלקוחות שכבר החלו לצבור ניקובים.
                </p>
             </div>
