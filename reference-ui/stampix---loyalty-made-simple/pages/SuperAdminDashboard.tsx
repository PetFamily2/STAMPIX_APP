...
const MOCK_MERCHANTS: MerchantStatus[] = [
  { 
    id: 'm1', name: 'קפה ארומה', category: 'קפה', revenue: '₪12,400', status: 'active', 
    logo: 'https://picsum.photos/100/100?u=10', customers: 450, punches: 1200, tier: 'Pro', 
    lastBillDate: '01/03/24', registrationDate: '15/12/2023', credits: 50,
    ownerEmail: 'manager@aroma-tlv.co.il', notes: 'לקוח VIP, דורש מענה מהיר.',
    toolUsage: { aiVideos: 4, marketingMessages: 124, scansPerMonth: 850 }
  },
  { 
    id: 'm2', name: 'פיצה פאמיליה', category: 'אוכל', revenue: '₪8,200', status: 'stuck', 
    logo: 'https://picsum.photos/100/100?u=11', customers: 210, punches: 840, tier: 'Starter', 
    lastBillDate: '05/03/24', registrationDate: '02/02/2024', credits: 0,
    ownerEmail: 'pizzahouse@gmail.com', notes: 'בעיה בסנכרון מול הקופה.',
    toolUsage: { aiVideos: 0, marketingMessages: 12, scansPerMonth: 120 }
  },
  { 
    id: 'm3', name: 'גולדה', category: 'גלידה', revenue: '₪19,000', status: 'pending', 
    logo: 'https://picsum.photos/100/100?u=13', customers: 890, punches: 3200, tier: 'Unlimited', 
    lastBillDate: '20/02/24', registrationDate: '10/01/2024', credits: 200,
    ownerEmail: 'office@golda-global.com', notes: 'סניף הדגל בירושלים.',
...
export default function SuperAdminDashboard({ onNavigate }: Props) {
  ...
  const [inspectedMerchant, setInspectedMerchant] = useState<MerchantStatus | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [actionLoading, setActionLoading] = useState<string | null>(null);
  const [showToast, setShowToast] = useState<string | null>(null);

  const filteredMerchants = MOCK_MERCHANTS.filter(m => 
    m.name.includes(searchQuery) || m.ownerEmail.includes(searchQuery)
  );
  ...
  const FeatureSuggestionCard = ({ title, icon, color }: any) => (
    <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex flex-col gap-3 group hover:bg-white/10 transition-all cursor-pointer">
       ...
       <div>
          ...
          <p className="text-[10px] text-gray-500 font-bold leading-tight">שלח הודעת הדרכה על השימוש בפיצ'ר זה</p>
       </div>
       <button 
    ...
    <div className="flex-1 flex flex-col bg-[#0a0f1a] text-white animate-in fade-in duration-500 font-sans pb-10">
      ...
      <header className="p-6 bg-[#111827]/80 backdrop-blur-xl sticky top-0 z-40 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
             <h1 className="font-black text-base tracking-tight">{inspectedMerchant ? inspectedMerchant.name : 'STAMPIX OS Admin'}</h1>
             <span className="text-[9px] text-gray-500 font-black uppercase tracking-widest">
               {inspectedMerchant ? `Internal ID: ${inspectedMerchant.id}` : 'Super User Management'}
             </span>
          </div>
        </div>
        {!inspectedMerchant && (
           <div className="flex gap-2">
             <button onClick={() => setActiveTab('stats')} className={`size-10 rounded-xl flex items-center justify-center transition-all ${activeTab === 'stats' ? 'bg-blue-600 text-white' : 'bg-white/5 text-gray-400'}`}>
                <span className="material-symbols-outlined">analytics</span>
             </button>
             <button onClick={() => setActiveTab('merchants')} className={`size-10 rounded-xl flex items-center justify-center transition-all ${activeTab === 'merchants' ? 'bg-blue-600 text-white' : 'bg-white/5 text-gray-400'}`}>
                <span className="material-symbols-outlined">storefront</span>
             </button>
           </div>
        )}
        <button onClick={() => onNavigate('splash')} className="size-10 bg-rose-500/10 text-rose-500 rounded-xl flex items-center justify-center">
              ...
              <div className="flex flex-col gap-6 animate-in slide-in-from-bottom duration-500">
                 <div className="grid grid-cols-2 gap-4">
                    <div className="bg-[#111827] p-5 rounded-[2rem] border border-white/5">
                       <span className="text-gray-500 text-[9px] font-black uppercase block mb-1">מחזור עסקאות</span>
                       <span className="text-3xl font-black text-emerald-400">₪342,800</span>
                    </div>
                    <div className="bg-[#111827] p-5 rounded-[2rem] border border-white/5">
                       <span className="text-gray-500 text-[9px] font-black uppercase block mb-1">ניקובים היום</span>
                       <span className="text-3xl font-black text-blue-500">1,240</span>
                    </div>
                 </div>

                 <section className="bg-[#111827] p-6 rounded-[2.5rem] border border-white/5">
                    <h3 className="font-black text-xs text-gray-400 uppercase tracking-widest mb-6 text-right">התראות מערכת דחופות</h3>
                    <div className="space-y-4">
                       <div className="flex flex-row-reverse items-center justify-between bg-rose-500/5 p-4 rounded-2xl border border-rose-500/10">
                          <div className="flex flex-row-reverse items-center gap-3">
                             ...
                             <div className="text-right">
                                <h4 className="text-sm font-bold">פיצה פאמיליה - חיוב נכשל</h4>
                                <p className="text-[10px] text-rose-500 font-medium">כרטיס האשראי פג תוקף</p>
                             </div>
                          </div>
                          <button onClick={() => setInspectedMerchant(MOCK_MERCHANTS[1])} className="text-[10px] font-black underline">טפל עכשיו</button>
                       </div>
                    </div>
                 </section>
              </div>
                    ...
                    {filteredMerchants.map(m => (
                      ...
                      >
                         <div className="flex flex-row-reverse items-center gap-4 text-right">
                            <img src={m.logo} className="size-14 rounded-2xl border border-white/10" alt={m.name} />
                            <div>
                               <h4 className="font-black text-lg">{m.name}</h4>
                               <span className="text-[10px] text-gray-500 font-bold">{m.tier} • {m.category}</span>
                            </div>
                         </div>
                         <div className="text-left">
                            <span className="text-emerald-400 font-black block">{m.revenue}</span>
                            <span className="text-[9px] text-gray-600 font-bold">{m.punches} ניקובים</span>
                         </div>
                      </div>
                    ))}
                 </div>
          ...
          <div className="flex flex-col gap-6 animate-in zoom-in duration-300">
             ...
             <section className="bg-gradient-to-br from-[#111827] to-[#1a2e44] p-8 rounded-[3rem] border border-white/10">
                <div className="flex flex-col items-center text-center">
                   <img src={inspectedMerchant.logo} className="size-20 rounded-[2rem] shadow-2xl mb-4 border-2 border-blue-500" />
                   <h2 className="text-2xl font-black">{inspectedMerchant.name}</h2>
                   <p className="text-gray-500 text-xs font-bold uppercase tracking-widest mt-1">חבר מועדון מ: {inspectedMerchant.registrationDate}</p>
                   
                   <div className="mt-6 w-full flex flex-col gap-2">
                      <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest text-right px-4">הערות פנימיות (אדמין)</label>
                      <textarea 
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#111827] p-6 rounded-[2.5rem] border border-white/5 flex flex-col gap-4">
                   <h3 className="text-[10px] font-black text-blue-400 uppercase tracking-widest text-right">אבטחה וגישה</h3>
                   <button 
                ...
                <div className="bg-[#111827] p-6 rounded-[2.5rem] border border-white/5 flex flex-col gap-4">
                   <h3 className="text-[10px] font-black text-emerald-400 uppercase tracking-widest text-right">פיננסים וחיוב</h3>
                   <button 
             ...
             <section className="bg-[#111827] p-6 rounded-[2.5rem] border border-white/5">
                <div className="flex flex-row-reverse items-center justify-between mb-6">
                   <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">זיכויים והטבות מערכת</h3>
                   <span className="bg-emerald-500 text-white text-[10px] font-black px-2 py-0.5 rounded uppercase">Balance: ₪{inspectedMerchant.credits}</span>
                </div>
                <div className="flex gap-3 flex-row-reverse">
                   ...
                   >
                      ...
                      <span className="text-[10px] font-black">הוסף זיכוי כספי</span>
                   </button>
                   <button 
             ...
             <section className="bg-[#111827] p-6 rounded-[2.5rem] border border-white/5">
                <h3 className="text-[10px] font-black text-blue-400 uppercase tracking-widest text-right mb-6">ניצול כלי האפליקציה</h3>
                <div className="grid grid-cols-3 gap-3 mb-8">
                   <div className="text-center">
                      <span className="text-lg font-black block">{inspectedMerchant.toolUsage.aiVideos}</span>
                      <span className="text-[9px] text-gray-500 font-bold">סרטוני AI</span>
                   </div>
                   <div className="text-center border-x border-white/5">
                      <span className="text-lg font-black block">{inspectedMerchant.toolUsage.marketingMessages}</span>
                      <span className="text-[9px] text-gray-500 font-bold">הודעות שיווק</span>
                   </div>
                   <div className="text-center">
                      <span className="text-lg font-black block">{inspectedMerchant.toolUsage.scansPerMonth}</span>
                      <span className="text-[9px] text-gray-500 font-bold">סריקות/חודש</span>
                   </div>
                </div>

                <div className="space-y-4">
                   <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">הצעות פיצ'רים לשליחה (Uplift)</h4>
                   <div className="grid grid-cols-2 gap-3">
                      <FeatureSuggestionCard title="יצירת וידאו AI" icon="movie_edit" color="bg-blue-500/20 text-blue-400" />
                      <FeatureSuggestionCard title="שימור לקוחות" icon="auto_awesome" color="bg-amber-500/20 text-amber-400" />
                      <FeatureSuggestionCard title="ניהול צוות" icon="badge" color="bg-emerald-500/20 text-emerald-400" />
                      <FeatureSuggestionCard title="אנליטיקה" icon="monitoring" color="bg-purple-500/20 text-purple-400" />
                   </div>
                </div>
             </section>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-[#111827]/90 backdrop-blur-md px-6 py-4 border-t border-white/5 flex justify-between items-center z-40">
         <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] text-emerald-500 font-black uppercase tracking-widest">Active Admin Session</span>
         </div>
         <div className="text-[9px] text-gray-600 font-black uppercase tracking-widest">
            {inspectedMerchant ? `Managing: ${inspectedMerchant.name}` : 'Stampix Platform v3.0'}
         </div>
      </footer>
    </div>
  );
