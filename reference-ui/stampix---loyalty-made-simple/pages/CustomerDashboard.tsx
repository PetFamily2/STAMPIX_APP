    ...
    <div className="flex-1 flex flex-col pb-24 overflow-y-auto">
        ...
        {isLoading ? (
          <div className="flex flex-col gap-4">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="h-40 bg-white/50 animate-pulse rounded-[2rem] border border-gray-100"
              ></div>
            ))}
          </div>
        ) : error ? (
          <div className="bg-rose-50 p-6 rounded-[2rem] text-center border border-rose-100">
            ...
            <button
              onClick={fetchCards}
              className="bg-white px-6 py-2 rounded-xl text-sm font-black border border-rose-200"
            >
              נסה שוב
            </button>
          </div>
          ...
          <div className="mt-10 flex flex-col items-center text-center px-10">
            <div className="size-20 bg-gray-50 rounded-full flex items-center justify-center mb-6">
              <span className="material-symbols-outlined text-gray-200 text-5xl">
                wallet
              </span>
            </div>
            <h3 className="text-xl font-black text-text-main">הארנק ריק...</h3>
            <p className="text-gray-400 text-sm mt-2 leading-relaxed">
              עדיין לא הצטרפת למועדונים. סרוק קוד QR של עסק כדי להתחיל!
            </p>
            <button
            ...
            {cards.map((card) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`size-16 rounded-2xl flex items-center justify-center text-white ${card.color || 'bg-blue-600'}`}
                  >
                    <span className="material-symbols-outlined text-3xl">
                      storefront
                    </span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {Array.from({ length: card.maxStamps }).map((_, i) => (
                        <div
                          key={i}
                          className={`size-6 rounded-full flex items-center justify-center border-2 ${i < card.currentStamps ? 'bg-blue-600 border-blue-600 text-white' : 'border-gray-100 bg-gray-50'}`}
                        >
                          {i < card.currentStamps ? (
                            ...
                            i === card.maxStamps - 1 && (
                              <span className="material-symbols-outlined text-[14px] text-gray-200">
                                redeem
                              </span>
                            )
                          )}
                ...
                <div
                  className={`absolute top-0 left-0 w-1.5 h-full ${card.color || 'bg-blue-600'}`}
                ></div>
              </div>
            ))}
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex justify-around items-end pb-8 pt-3 z-10 px-4 shadow-[0_-5px_15px_rgba(0,0,0,0.02)]">
        <button
          ...
          className={`flex flex-col items-center gap-1 ${activeTab === 'wallet' ? 'text-blue-600' : 'text-gray-300'}`}
        >
          <span
            className={`material-symbols-outlined text-[28px] ${activeTab === 'wallet' ? 'material-symbols-filled' : ''}`}
          >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 ${activeTab === 'rewards' ? 'text-blue-600' : 'text-gray-300'}`}
        >
          <span
            className={`material-symbols-outlined text-[28px] ${activeTab === 'rewards' ? 'material-symbols-filled' : ''}`}
          >
            redeem
          </span>
        ...
        <button
          onClick={() => onNavigate('discovery')}
          className={`flex flex-col items-center gap-1 ${activeTab === 'discovery' ? 'text-blue-600' : 'text-gray-300'}`}
        >
          <span
            className={`material-symbols-outlined text-[28px] ${activeTab === 'discovery' ? 'material-symbols-filled' : ''}`}
          >
            explore
          </span>
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 ${activeTab === 'settings' ? 'text-blue-600' : 'text-gray-300'}`}
        >
          <span
            className={`material-symbols-outlined text-[28px] ${activeTab === 'settings' ? 'material-symbols-filled' : ''}`}
          >
            menu
          </span>
