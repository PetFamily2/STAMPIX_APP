    ...
    <div className="flex-1 flex flex-col animate-in slide-in-from-left duration-300 pb-10">
      ...
      <main className="px-6 flex flex-col gap-8 mt-4">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-primary/5 border border-gray-50 flex flex-col items-center text-center relative overflow-hidden">
          <div
            className={`absolute top-0 left-0 w-full h-2 ${card.color}`}
          ></div>
          <img
            src={card.logo}
            className="size-24 rounded-3xl object-cover mb-4 shadow-md ring-4 ring-white"
          ...
          <p className="text-primary font-bold mb-6">
            צברת {card.currentStamps} מתוך {card.maxStamps} חתימות
          </p>
        ...
        <section className="bg-primary/5 rounded-3xl p-6 border border-primary/10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="font-bold text-text-main text-lg">
              הפרס שלך: {card.reward}
            </h3>
          </div>
          <p className="text-gray-500 text-sm leading-relaxed">
            ברגע שתשלים את כל הניקובים בכרטיס, תוכל לממש את ההטבה שלך ישירות מול
            המוכר בקופה.
          </p>
        </section>
          ...
          <div className="flex flex-col gap-4">
            {MOCK_HISTORY.map((item, idx) => (
              <div key={item.id} className="flex gap-4 relative">
                ...
                <div
                  className={`size-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${
                    item.type === 'reward'
