  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-mint-bg shadow-2xl overflow-x-hidden flex flex-col relative">
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        {currentPage === 'plan_selection' && (
          <PlanSelection onSelect={handlePlanSelect} />
        )}

        {currentPage === 'customer_home' && (
          <CustomerDashboard
        ...
        {currentPage === 'discovery' && (
          <Discovery onNavigate={setCurrentPage} />
        )}

        {currentPage === 'merchant_home' && (
          <MerchantDashboard onNavigate={setCurrentPage} />
        )}
        {currentPage === 'merchant_analytics' && (
          <MerchantAnalytics onBack={() => setCurrentPage('merchant_home')} />
        )}
        ...
        {currentPage === 'super_admin_home' && (
          <SuperAdminDashboard onNavigate={setCurrentPage} />
        )}
        {currentPage === 'employee_mgmt' && (
          <EmployeeManagement onBack={() => setCurrentPage('merchant_home')} />
        )}
        {currentPage === 'merchant_profile' && (
        ...
        {currentPage === 'card_details' && selectedCard && (
          <CardDetails
            card={selectedCard}
            onBack={() => setCurrentPage('customer_home')}
          />
        )}

        {currentPage === 'scanner' && (
          <Scanner onBack={() => setCurrentPage('merchant_home')} />
        )}
        {currentPage === 'customer_scanner' && (
          <CustomerScanner onBack={() => setCurrentPage('customer_home')} />
        )}
        {currentPage === 'merchant_qr' && (
          <MerchantQR onBack={() => setCurrentPage('merchant_home')} />
        )}
        {currentPage === 'video_generator' && (
          <VideoGenerator onBack={() => setCurrentPage('merchant_home')} />
        )}
